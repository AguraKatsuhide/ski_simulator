...
void processFrame() //실질적인 트레킹 기능을 행하는 부분
{
	...
	if(g_HX!=0 && g_HY!=0 && g_RX!=0 && g_RY!=0 && distpoints(g_Under_irAX,g_Under_irAY,g_RX,g_RY)<=3){ //스키 점프 자세 정의
		...
		double diffHLJump = g_HLdistJump-HLdistJump;//머리와 오른다리의 거리의 기준 과 현재 값의 차이.
		...
		if(g_initJumpValue == true && HLdistJump <= (g_HLdistJump)*(0.7) || g_initJumpValue == true && diffHLJump<10 && HLdistJump<=120){ //HLdistJump<=120
			...
			sprintf(g_strV,"Current Pose : Downhill (3)");// 활강 자세.
		...
		if(g_initJumpValue == true && HLdistJump >= (g_HLdistJump)*(1.1) || g_initJumpValue == true && diffHLJump<10 && HLdistJump>120){  //HLdistJump>120
			...
			sprintf(g_strV,"Current Pose : Jump (4)");// 스키점프 자세.
		...
		if(g_Under_irAX<256){
			    ...
			    g_HX = g_HX+(39*(256-g_Under_irAX)/256); // 화각에 맞춰 x 축 길이 조절.
				g_3dX =g_3dX+(19*(256-g_Under_irAX)/256);
			...
			if(g_Under_irAX>256){
				...
				g_HX = g_HX-(39*(g_Under_irAX-256)/256); // 화각에 맞춰 x 축 길이 조절.
				g_3dX =g_3dX-(19*(g_Under_irAX-256)/256);
		...
		{// stand 자세 값 저장 후 다른 자세 구분.
			...
			Head3d=distpoints(g_HX,g_HY,g_3dX,g_3dY);// 현재 머리와 3d 중점 거리 
			Leg3d=distpoints(g_Under_irAX,g_Under_irAY,g_3dX,g_3dY);// 현재 다리와 3d 중점 거리
			HLdist=distpoints(g_HX,g_HY,g_Under_irAX,g_Under_irAY);// 현재 다리와 3d 중점 거리
			...
			double L3d_x = g_Under_irAX-g_3dX;
			L3d_x = fabs(L3d_x);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if(  diffHLdist <= 3 /*&&  Head3d >=10*/ /*&& g_init3dz >= g_3dZ*/ && diffdistL <10/*&& diff3dz<=50*/ && L3d_x<=12 && g_HX>g_LX &&g_HX<g_RX){
			...
			//if( g_HLdist > HLdist && Head3d < 10  && diffdistL<5 && g_init3dz < g_3dZ && L3d_x<=12 ){
			...
			if( g_HLdist > HLdist && diffdistL>=10 && (g_init3dz - g_3dZ)>1 && L3d_x<=12 && g_HX>g_LX &&g_HX<g_RX){
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if( g_HY!=0 && g_LY!=0 && g_RY!=0 && g_3dX<g_Under_irAX && (g_Under_irAX-g_3dX)>12 && g_HX<g_LX || // 3차원 중심점과 양발 중점 비교.
			...
			if(g_HY!=0 && g_LY!=0 && g_RY!=0 && g_3dX>g_Under_irAX && (g_3dX-g_Under_irAX)>12 && g_HX>g_RX ||  // 중심점과 양발 중점 비교.
